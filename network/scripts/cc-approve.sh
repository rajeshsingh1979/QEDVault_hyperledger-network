...
peer lifecycle chaincode approveformyorg -o $ORDERER_ADDRESS  --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_ID --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} --signature-policy 'OR("Org1MSP.peer")'
set +x